...
output                  = {logs_dir}/{year}_{job_name}.out
error                   = {logs_dir}/{year}_{job_name}.err
log                     = {logs_dir}/{year}_{job_name}.log
...
# Upload hash using xrdcp to avoid write lock
xrdcp -f commithash.txt ${{eos_target}}/jobchecks/commithash_{job_index}.txt || true

# ========= 3) Install Write it under HOME/.local to avoid polluting the system =========
pip install --user -e .
...
# ========= 5) Upload outputs, all using xrdcp =========
...
def generate_for_one_combo(
    ...
    for job_idx in range(njobs):
        ...
        render_jdl(
            ...
            job_name=f"{subsample}_{job_idx}",
            year=year,
        )
...
def generate_run_submit_sh(tag: str, nano_version: str, region: str, year: str, eos_user_path: str):
    submit_dir = Path("condor") / "skimmer" / f"{tag}_{nano_version}_{region}"
    run_sh_path = submit_dir / f"run_submit_{year}.sh"
    ...
    lines.append(
        f'eos_base="/eos/user/{eos_user_path}/bbtautau/skimmer/{tag}_{nano_version}_{region}/{year}"'
    )
    lines.append("shopt -s nullglob")

    lines.append("for f in " + str(submit_dir) + "/*_eos.sub; do")
    lines.append('    bn=$(basename "$f")')
    lines.append(f'    if [[ "$bn" =~ ^{year}_(.+)_([0-9]+)_eos\.sub$ ]]; then')
    lines.append('        sample_name="${BASH_REMATCH[1]}"')
    lines.append("    else")
    lines.append('        echo "Skipping unexpected file name format: $bn"')
    lines.append("        continue")
    lines.append("    fi")

    lines.append('    xrdfs root://eosuser.cern.ch/ mkdir -p "$eos_base/$sample_name/pickles"')
    lines.append('    xrdfs root://eosuser.cern.ch/ mkdir -p "$eos_base/$sample_name/parquet"')
    lines.append('    xrdfs root://eosuser.cern.ch/ mkdir -p "$eos_base/$sample_name/root"')
    lines.append('    xrdfs root://eosuser.cern.ch/ mkdir -p "$eos_base/$sample_name/jobchecks"')
    lines.append('    echo "Submitting $f"')
    lines.append('    condor_submit "$f"')
    lines.append("done")

    run_sh_path.write_text("\n".join(lines), encoding="utf-8")
    Path.chmod(run_sh_path, 0o755)
...
def main():
    ...
    parser.add_argument(
        "--singularity-image",
        default="/cvmfs/unpacked.cern.ch/registry.hub.docker.com/coffeateam/coffea-dask-almalinux9:2025.9.0-py3.11",
    )
