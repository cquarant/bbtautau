...
class bbtautauSkimmerTTGenMatching(SkimmerABC):
    """
    ...
    def __init__(
        ...
        reco_gen_lepton_dr: float = 0.2,
        num_gen_top_b: int = 2,
        num_genjet_top_b: int = 4,
        num_gen_top_leptons: int = 2,
    ):
        super().__init__()
        ...
        self._reco_gen_lepton_dr = reco_gen_lepton_dr
        self._num_gen_top_b = num_gen_top_b
        self._num_genjet_top_b = num_genjet_top_b
        self._num_gen_top_leptons = num_gen_top_leptons
    ...
    def get_ttbar_gen_matching_vars(
        ...
        2. A GenJet is tagged as from top-b hadronization if its nearest such b quark is
           within self._genjet_topb_dr. If GenJet.partonFlavour exists, require |partonFlavour| == 5.
        ...
        # Keep the original per-event GenJet index before slicing.
    ...
    def _empty_ttbar_gen_lepton_matching_vars(self, n_events: int, num_leptons: int) -> dict:
        """Return PAD_VAL-filled gen-lepton/matching branches for data or samples without GenPart."""
        out = {}
        for label in ["Electron", "Muon"]:
            out[f"nGenTop{label}"] = np.zeros(n_events, dtype=np.int32)
            for var in [
                "Pt",
                "Eta",
                "Phi",
                ...
                out[f"GenTop{label}{var}"] = np.full((n_events, self._num_gen_top_leptons), PAD_VAL)
            out[f"{label}IsMatchedToTopGen{label}"] = np.zeros(
                (n_events, num_leptons), dtype=np.int32
            )
            for var in [
                "Pt",
                "Eta",
                ...
                out[f"{label}MatchedTopGen{label}{var}"] = np.full((n_events, num_leptons), PAD_VAL)
        return out

    def _select_gen_top_leptons(self, genparts: ak.Array, abs_pdgid: int) -> ak.Array:
        """Select final e/mu from top decay.

        Direct W decays are selected with:
            top -> W -> e/mu
        Leptonic tau chains are also kept with:
            top -> W -> tau -> e/mu
        and flagged with fromTau=1.
        """
        parent = genparts.distinctParent
        grandparent = parent.distinctParent
        greatgrandparent = grandparent.distinctParent

        parent_pdgid = ak.fill_none(parent.pdgId, 0)
        grandparent_pdgid = ak.fill_none(grandparent.pdgId, 0)
        greatgrandparent_pdgid = ak.fill_none(greatgrandparent.pdgId, 0)

        is_final_lepton = (abs(genparts.pdgId) == abs_pdgid) & genparts.hasFlags(["isLastCopy"])

        from_direct_w_top = (abs(parent_pdgid) == 24) & (abs(grandparent_pdgid) == 6)
        from_tau_w_top = (
            (abs(parent_pdgid) == 15)
            & (abs(grandparent_pdgid) == 24)
            & (abs(greatgrandparent_pdgid) == 6)
        )

        is_top_lepton = ak.fill_none(is_final_lepton & (from_direct_w_top | from_tau_w_top), False)
        gen_top_lepton = genparts[is_top_lepton]

        # Recompute ancestry fields after slicing so each saved gen lepton carries the relevant truth info.
        parent_pdgid_sel = ak.fill_none(gen_top_lepton.distinctParent.pdgId, 0)
        grandparent_pdgid_sel = ak.fill_none(gen_top_lepton.distinctParent.distinctParent.pdgId, 0)
        from_tau_sel = abs(parent_pdgid_sel) == 15

        gen_top_lepton = ak.with_field(gen_top_lepton, parent_pdgid_sel, "topLeptonParentPdgId")
        gen_top_lepton = ak.with_field(
            gen_top_lepton, grandparent_pdgid_sel, "topLeptonGrandparentPdgId"
        )
        gen_top_lepton = ak.with_field(
            gen_top_lepton, ak.values_astype(from_tau_sel, np.int32), "topLeptonFromTau"
        )
        gen_top_lepton = gen_top_lepton[ak.argsort(gen_top_lepton.pt, axis=1, ascending=False)]
        return gen_top_lepton

    def get_ttbar_gen_lepton_matching_vars(
        self,
        events: ak.Array,
        electrons: ak.Array,
        muons: ak.Array,
        ...
        """Build gen e/mu from top-decay branches and reco lepton matching branches.

        The matching is done independently for reco electrons and reco muons with nearest ΔR.
        Saved branches named ElectronMatchedTopGenElectron* and MuonMatchedTopGenMuon* are
        properties of the matched gen lepton, while the corresponding *DR branch gives the
        reco-gen angular distance.
        """
        n_events = len(events)
        if "GenPart" not in events.fields:
            return self._empty_ttbar_gen_lepton_matching_vars(n_events, num_leptons)

        genparts = events.GenPart
        gen_top_electrons = self._select_gen_top_leptons(genparts, 11)
        gen_top_muons = self._select_gen_top_leptons(genparts, 13)

        matched_gen_electron, electron_gen_dr = electrons.nearest(
            gen_top_electrons,
            axis=1,
            return_metric=True,
            threshold=self._reco_gen_lepton_dr,
        )
        electron_is_matched = ak.fill_none(electron_gen_dr < self._reco_gen_lepton_dr, False)

        matched_gen_muon, muon_gen_dr = muons.nearest(
            gen_top_muons,
            axis=1,
            return_metric=True,
            threshold=self._reco_gen_lepton_dr,
        )
        muon_is_matched = ak.fill_none(muon_gen_dr < self._reco_gen_lepton_dr, False)

        return {
            "nGenTopElectron": ak.num(gen_top_electrons).to_numpy(),
            "GenTopElectronPt": pad_val(gen_top_electrons.pt, self._num_gen_top_leptons, axis=1),
            "GenTopElectronEta": pad_val(gen_top_electrons.eta, self._num_gen_top_leptons, axis=1),
            "GenTopElectronPhi": pad_val(gen_top_electrons.phi, self._num_gen_top_leptons, axis=1),
            "GenTopElectronMass": pad_val(
                gen_top_electrons.mass, self._num_gen_top_leptons, axis=1
            ),
            "GenTopElectronPdgId": pad_val(
                gen_top_electrons.pdgId, self._num_gen_top_leptons, axis=1
            ),
            "GenTopElectronParentPdgId": pad_val(
                gen_top_electrons.topLeptonParentPdgId, self._num_gen_top_leptons, axis=1
            ),
            "GenTopElectronGrandparentPdgId": pad_val(
                gen_top_electrons.topLeptonGrandparentPdgId, self._num_gen_top_leptons, axis=1
            ),
            "GenTopElectronFromTau": pad_val(
                gen_top_electrons.topLeptonFromTau, self._num_gen_top_leptons, axis=1
            ),
            "ElectronIsMatchedToTopGenElectron": pad_val(
                electron_is_matched, num_leptons, False, axis=1
            ).astype(int),
            "ElectronMatchedTopGenElectronPt": pad_val(
                matched_gen_electron.pt, num_leptons, axis=1
            ),
            "ElectronMatchedTopGenElectronEta": pad_val(
                matched_gen_electron.eta, num_leptons, axis=1
            ),
            "ElectronMatchedTopGenElectronPhi": pad_val(
                matched_gen_electron.phi, num_leptons, axis=1
            ),
            "ElectronMatchedTopGenElectronMass": pad_val(
                matched_gen_electron.mass, num_leptons, axis=1
            ),
            "ElectronMatchedTopGenElectronPdgId": pad_val(
                matched_gen_electron.pdgId, num_leptons, axis=1
            ),
            "ElectronMatchedTopGenElectronParentPdgId": pad_val(
                matched_gen_electron.topLeptonParentPdgId, num_leptons, axis=1
            ),
            "ElectronMatchedTopGenElectronGrandparentPdgId": pad_val(
                matched_gen_electron.topLeptonGrandparentPdgId, num_leptons, axis=1
            ),
            "ElectronMatchedTopGenElectronDR": pad_val(electron_gen_dr, num_leptons, axis=1),
            "ElectronMatchedTopGenElectronFromTau": pad_val(
                matched_gen_electron.topLeptonFromTau, num_leptons, axis=1
            ),
            "nGenTopMuon": ak.num(gen_top_muons).to_numpy(),
            "GenTopMuonPt": pad_val(gen_top_muons.pt, self._num_gen_top_leptons, axis=1),
            "GenTopMuonEta": pad_val(gen_top_muons.eta, self._num_gen_top_leptons, axis=1),
            "GenTopMuonPhi": pad_val(gen_top_muons.phi, self._num_gen_top_leptons, axis=1),
            "GenTopMuonMass": pad_val(gen_top_muons.mass, self._num_gen_top_leptons, axis=1),
            "GenTopMuonPdgId": pad_val(gen_top_muons.pdgId, self._num_gen_top_leptons, axis=1),
            "GenTopMuonParentPdgId": pad_val(
                gen_top_muons.topLeptonParentPdgId, self._num_gen_top_leptons, axis=1
            ),
            "GenTopMuonGrandparentPdgId": pad_val(
                gen_top_muons.topLeptonGrandparentPdgId, self._num_gen_top_leptons, axis=1
            ),
            "GenTopMuonFromTau": pad_val(
                gen_top_muons.topLeptonFromTau, self._num_gen_top_leptons, axis=1
            ),
            "MuonIsMatchedToTopGenMuon": pad_val(
                muon_is_matched, num_leptons, False, axis=1
            ).astype(int),
            "MuonMatchedTopGenMuonPt": pad_val(matched_gen_muon.pt, num_leptons, axis=1),
            "MuonMatchedTopGenMuonEta": pad_val(matched_gen_muon.eta, num_leptons, axis=1),
            "MuonMatchedTopGenMuonPhi": pad_val(matched_gen_muon.phi, num_leptons, axis=1),
            "MuonMatchedTopGenMuonMass": pad_val(matched_gen_muon.mass, num_leptons, axis=1),
            "MuonMatchedTopGenMuonPdgId": pad_val(matched_gen_muon.pdgId, num_leptons, axis=1),
            "MuonMatchedTopGenMuonParentPdgId": pad_val(
                matched_gen_muon.topLeptonParentPdgId, num_leptons, axis=1
            ),
            "MuonMatchedTopGenMuonGrandparentPdgId": pad_val(
                matched_gen_muon.topLeptonGrandparentPdgId, num_leptons, axis=1
            ),
            "MuonMatchedTopGenMuonDR": pad_val(muon_gen_dr, num_leptons, axis=1),
            "MuonMatchedTopGenMuonFromTau": pad_val(
                matched_gen_muon.topLeptonFromTau, num_leptons, axis=1
            ),
        }
    ...
    def process(self, events: ak.Array):
        ...
        taus, ttrigvars = objects.good_taus(events, events.Tau, year=year)
        ...
        ttbarGenLeptonMatchVars = self.get_ttbar_gen_lepton_matching_vars(
            events,
            electrons,
            muons,
            num_leptons,
